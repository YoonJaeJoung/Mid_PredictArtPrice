...
- **AI Art Generation (NanoBanana)**: I developed a sophisticated generative system that translates market data into aesthetic imagery. 
    1.  **Percentile Division**: Within each visual cluster, the system calculates price quartiles (25th, 50th, and 75th percentiles) to categorize artworks into four economic tiers: *Economy*, *Mid-Market*, *Premium*, and *Masterpiece*.
    2.  **Structured Prompt Engineering**: For each tier, a complex prompt is assembled consisting of:
        - **Cluster Vibe**: A semantic summary of the cluster's visual theme (generated by Gemini 3 Flash).
        - **Market Context**: Tier-specific descriptions of the "value" characteristics.
        - **Visual Anchors**: 10 actual artworks from the auction dataset that fall within that price tier, used as strict stylistic and compositional references for the generative model.
    3.  **Imagery Production**: These data-driven prompts are processed by **Gemini Nanobanana 2** to "engineer" high-value aesthetic characteristics, allowing us to visualize what "expensive" art looks like within specific genres.
